export default [
  {
    question: 'Spring Cloud 5大组件有哪些？',
    answer: '早期我们一般认为的Spring Cloud五大组件是 \n' +
      '- Eureka   : 注册中心\n' +
      '- Ribbon  : 负载均衡\n' +
      '- Feign     : 远程调用\n' +
      '- Hystrix :  服务熔断\n' +
    ...
    question: '服务注册和发现是什么意思？Spring Cloud 如何实现服务注册发现？',
    answer: '我理解的是主要三块大功能，分别是服务注册 、服务发现、服务状态监控\n' +
      '我们当时项目采用的eureka作为注册中心，这个也是spring cloud体系中的一个核心组件\n' +
      '服务注册：服务提供者需要把自己的信息注册到eureka，由eureka来保存这些信息，比如服务名称、ip、端口等等\n' +
      '服务发现：消费者向eureka拉取服务列表信息，如果服务提供者有集群，则消费者会利用负载均衡算法，选择一个发起调用\n' +
      '服务监控：服务提供者会每隔30秒向eureka发送心跳，报告健康状态，如果eureka服务90秒没接收到心跳，从eureka中剔除\n'
  }, {
    question: '我看你之前也用过nacos、你能说下nacos与eureka的区别？',
    answer: '- 共同点：Nacos与eureka都支持服务注册和服务拉取，都支持服务提供者心跳方式做健康检测\n' +
      '- Nacos与Eureka的区别\n' +
    ...
    question: '你们项目负载均衡如何实现的？',
    answer: '在服务调用过程中的负载均衡一般使用SpringCloud的Ribbon 组件实现  , Feign的底层已经自动集成了Ribbon  , 使用起来非常简单\n' +
      '当发起远程调用时，ribbon先从注册中心拉取服务地址列表，然后按照一定的路由策略选择一个发起远程调用，一般的调用策略是轮询'
  }, {
    question: 'Ribbon负载均衡策略有哪些？',
    answer: '- RoundRobinRule：简单轮询服务列表来选择服务器\n' +
      '- WeightedResponseTimeRule：按照权重来选择服务器，响应时间越长，权重越小\n' +
      '- RandomRule：随机选择一个可用的服务器\n' +
      '- ZoneAvoidanceRule：区域敏感策略，以区域可用的服务器为基础进行服务器的选择。使用Zone对服务器进行分类，这个Zone可以理解为一个机房、一个机架等。而后再对Zone内的多个服务做轮询(默认)'
  }, {
    question: '如果想自定义负载均衡策略如何实现？',
    answer: '提供了两种方式：\n' +
      '1、创建类实现IRule接口，可以指定负载均衡策略，这个是全局的，对所有的远程调用都起作用\n' +
      '2、在客户端的配置文件中，可以配置某一个服务调用的负载均衡策略，只是对配置的这个服务生效远程调用'
  }, {
    question: '什么是服务雪崩，怎么解决这个问题？',
    answer: '服务雪崩是指一个服务失败，导致整条链路的服务都失败的情形，一般我们在项目解决的话就是两种方案，第一个是服务降级，第二个是服务熔断，如果流量太大的话，可以考虑限流\n' +
    ...
    question: '你们的微服务是怎么监控的？',
    answer: '我们项目中采用的skywalking进行监控的\n' +
      '1，skywalking主要可以监控接口、服务、物理实例的一些状态。特别是在压测的时候可以看到众多服务中哪些服务和接口比较慢，我们可以针对性的分析和优化。\n' +
      '2，我们还在skywalking设置了告警规则，特别是在项目上线以后，如果报错，我们分别设置了可以给相关负责人发短信和发邮件，第一时间知道项目的bug情况，第一时间修复'
  }, {
    question: '你们项目中有没有做过限流？怎么做的？',
    answer: '我当时做的xx项目，采用就是微服务的架构，因为xx因为，应该会有突发流量，最大QPS可以达到2000，但是服务支撑不住，我们项目都通过压测最多可以支撑1200QPS。因为我们平时的QPS也就不到100，为了解决这些突发流量，所以采用了限流。\n' +
    ...
    question: '什么是CAP理论？',
    answer: 'CAP主要是在分布式项目下的一个理论。包含了三项，一致性、可用性、分区容错性\n' +
    ...
    question: '为什么分布式系统中无法同时保证一致性和可用性？',
    answer: '首先一个前提，对于分布式系统而言，分区容错性是一个最基本的要求，因此基本上我们在设计分布式系统的时候只能从一致性（C）和可用性（A）之间进行取舍。\n' +
      '如果保证了一致性（C）：对于节点N1和N2，当往N1里写数据时，N2上的操作必须被暂停，只有当N1同步数据到N2时才能对N2进行读写请求，在N2被暂停操作期间客户端提交的请求会收到失败或超时。显然，这与可用性是相悖的。\n' +
      '如果保证了可用性（A）：那就不能暂停N2的读写操作，但同时N1在写数据的话，这就违背了一致性的要求。'
  }, {
    question: '什么是BASE理论？',
    answer: '这个也是CAP分布式系统设计理论\n' +
    ...
    question: '你们采用哪种分布式事务解决方案？',
    answer: '我们当时是xx项目，主要使用到的seata的at模式解决的分布式事务\n' +
      'seata的AT模型分为两个阶段：\n' +
    ...
    answer: '我们当时有一个xx项目的下单操作，采用的token+redis实现的，流程是这样的\n' +
    ...
    question: 'xxl-job路由策略有哪些？',
    answer: 'xxl-job提供了很多的路由策略，我们平时用的较多就是：轮询、故障转移、分片广播…'
  }, {
    question: 'xxl-job任务执行失败怎么解决？',
    answer: '第一：路由策略选择故障转移，优先使用健康的实例来执行任务\n' +
      '第二，如果还有失败的，我们在创建任务时，可以设置重试次数\n' +
      '第三，如果还有失败的，就可以查看日志或者配置邮件告警来通知相关负责人解决'
  }, {
    question: '如果有大数据量的任务同时都需要执行，怎么解决？',
    answer: '我们会让部署多个实例，共同去执行这些批量的任务，其中任务的路由策略是分片广播\n' +
